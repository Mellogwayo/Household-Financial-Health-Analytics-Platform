...
```{r}
elastic_net_model$lambda
```


```{r}
# Find the closest lambda value
closest_lambda <- elastic_net_model$lambda[which.min(abs(elastic_net_model$lambda - best_lambda))]

# Now extract coefficients
coef_matrix <- as.matrix(coef(elastic_net_model, s = closest_lambda))

```

```{r}
result <- try(coef(elastic_net_model, s = best_lambda), silent = TRUE)
print(result)
```


```{r}
coef_matrix <- as.matrix(coef(elastic_net_model, s = "lambda.min"))
...
```{r}
# Manually define a smaller lambda sequence
lambda_seq <- 10^seq(3, -2, by = -0.1)

elastic_net_model2 <- cv.glmnet(
  x = train_matrix,
  y = train$jkl_fihhmnnet1_dv,
  alpha = 0.5,
  lambda = lambda_seq
)

# Try extracting coefficients again
coef_matrix <- as.matrix(coef(elastic_net_model2, s = "lambda.min"))
...
```{r}
# Smaller lambda range (less regularization)
...
# Extract coefficients at new lambda.min
coef_matrix <- as.matrix(coef(elastic_net_model2, s = "lambda.min"))
